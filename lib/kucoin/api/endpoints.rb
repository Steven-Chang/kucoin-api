  ...
  module Api
    ...
    module Endpoints
      ...
      def self.endpoint_method(client_klass, name, result, parent_var = nil)
        child_endpoint_klass = Endpoints.get_klass(name, parent_var&.class)

        if parent_var
          parent_var.define_singleton_method name do
            endpoint_var = "@#{name}"
            var = instance_variable_get(endpoint_var) || instance_variable_set(endpoint_var,
                                                                               child_endpoint_klass.new(parent_var.client))
      ...
      def generate_endpoint_methods(result: ENDPOINTS, parent_var: nil)
        result.each do |endpoint_name, _result|
          Kucoin::Api::Endpoints.endpoint_method(self, endpoint_name, _result, parent_var) if _result.is_a?(Hash)
        end
      end
    end
  end
